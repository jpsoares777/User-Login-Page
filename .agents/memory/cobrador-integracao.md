...
**pagos/noPagos do snapshot NÃO podem derivar de `cobrados`/`ausentes`:** esses arrays são zerados ao reabrir o caixa no mesmo dia (fechamento faz `setCobrados([])`), então a marcação "Sem pagamento" some do snapshot ao vivo (aparecia pagos:0 não pagos:0 mesmo com cliente marcado). Regra robusta: `pagos` = `cobradosValores.filter(valor>0).length` (persiste mesmo após quitar/sair de elegíveis); `noPagos` = elegíveis do dia com valor 0 (pendentes + "Sem pagamento"). Assim independe do bookkeeping volátil e "não pagos" já inclui pendentes não visitados. **Nota de rendering:** a exibição do rg SUBSTITUI o texto casado por "n" — o metodo real de sem-pagamento é a string `"Sem pagamento"`, não `"n"`; conferir com um grep cujo padrão NÃO case o literal.
