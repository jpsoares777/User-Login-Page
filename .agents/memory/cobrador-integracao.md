...
## Aba "Pagamentos" da web = linhas por cliente dentro do snapshot
A aba `Liq. Diária > Pagamentos` (dashboard.tsx `PagamentosContent`) mostra dados REAIS por cliente da rota SELECIONADA (`selectedRota`), não uma lista global. Mecanismo: o app inclui um array `pagamentos: PagamentoRow[]` DENTRO do `DadosSnapshot` em `buildDadosSnapshot`; como `/caixa/fechamento-rota` faz passthrough via `...snapshot`, o array chega à web sem tocar na API. `PagamentosContent` lê `rd.pagamentos`. Universo das linhas = `clientes` + `clientesAdicionaisHoje` (dedupe por id), filtro `saldo>0 OU pagou hoje`.

**Gating do demo (armadilha):** o array demo `pagamentosData` (24 clientes fixos: GEILSON, ALINE, MARIANA..., fecha "2026-05-25", "Operacion Masiva", valor 0,00) SÓ pode aparecer quando NENHUMA rota está selecionada. Com rota selecionada, usar `usandoReal = !!rota || !!realRows` e mapear de `realRows ?? []` — ou seja, dados reais da rota (tabela VAZIA se o snapshot ainda não trouxe `pagamentos`), NUNCA o demo. Sintoma do bug: rota São Bento (2 clientes) mostrava os 24 demo porque o fallback era incondicional quando `rd.pagamentos` vinha ausente.

**Semântica dos campos (derivada da imagem de referência do sistema original):** `tipo` = "Cuota"(Parcela)/"Abono"/"S/Pago" (do `metodo` do pagamento de hoje); `formaPago` = "Efectivo"(Dinheiro)/"Transferência"(PIX) — a web usa os rótulos ESPANHOL, não "Dinheiro"; `pagadas`/`restantes` formatados com 1 casa (`.toFixed(1)` → "1.0"); **`valorEmprestado` (col. "Valor Prod.") = `parcela * totalParcelas`** (total do contrato com juros), NÃO o principal. `status` bom/medio/ruim (cor do nome) = razão parcelasPagas/totalParcelas.
