...
**Dedupe (importante):** reimportar a mesma planilha gera novos comandos/ids. O app deduplica por id, por `consecutivo` e — quando a planilha não tem consecutivo — por chave composta nome+telefone+endereço normalizada. **Armadilha:** deduplicar SÓ a lista de clientes não basta — cobrados/cobradosValores/ordem/histórico devem usar o id do cliente EXISTENTE (não o id novo do comando), senão acumulam ids fantasmas (ex.: "Cobranças 8/4"). Há um effect de saneamento que filtra cobrados por ids conhecidos e persiste.
