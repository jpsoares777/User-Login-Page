...
const gastosIngresosData = MONTHS.map((m, i) => ({
  mes: m,
  Rendimentos: [3800, 4200, 8500, 3200, 6400, 7100, 5900, 4600, 9800, 5700, 4100, 3500][i],
  Despesas:    [1200, 1800, 2500, 1800, 2200, 2900, 2100, 1700, 3400, 2000, 1500, 1300][i],
}));
        ...
        <ChartCard {...cardCommon}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={rendVsDespChart} margin={{ top: 14, right: 20, left: 0, bottom: 4 }}>
              ...
              <Tooltip contentStyle={{ fontSize: 10 }} formatter={(v: number) => `$ ${v.toLocaleString("pt-BR")}`} />
              <Legend iconSize={7} iconType="circle" wrapperStyle={{ fontSize: 8, paddingTop: 2 }} />
              <Bar dataKey="Rendimentos" fill="#5b9bd5" maxBarSize={20} shape={<Bar3D depth={10} />} />
              <Bar dataKey="Despesas" fill="#2c2c2c" maxBarSize={20} shape={<Bar3D depth={10} />} />
            </BarChart>
          </ResponsiveContainer>
        </ChartCard>
      ...
      {activeMain === "Liq. Períodos" && activeSubPeriodos === "Liquidação" && (
        <div className="flex items-center h-11 px-3 gap-2 shrink-0" style={{ background: "#f8f9fa", borderBottom: "1px solid #e0e0e0" }}>
          ...
          <button onClick={() => { setBuscarRotaForm(emptyBuscarRota); setBuscarRotaOpen(true); }} className="flex items-center gap-1.5 px-3 h-8 text-sm font-medium rounded" style={{ background: "#2563eb", color: "#fff" }}>
            ...
            <span className="text-xs bg-white/25 rounded-full w-5 h-5 flex items-center justify-center font-bold">{estadosData[selectedEstado]?.length ?? 0}</span>
          </button>
        </div>
      )}
              ...
              {!selectedRota ? (
                ...
                return (<>
                  ...
                  <Row label="Clientes Novos" index={1}>
                    <PersonIcon /> <strong className="text-gray-800">{rd.clientesNovos}</strong> <span className="text-gray-400 text-[11px]">({rd.clientesNovosRegulares ?? rd.clientesNovos}/{rd.clientesNovosAdiantados ?? 0})</span>
                  </Row>
                  ...
                  <Row label="Recebido Hoje" index={3}>
