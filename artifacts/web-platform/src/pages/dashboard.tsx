...
// ── Liq. Períodos – Despesas (Egresos + Retiros) ─────────────────────────────
const egresosData = [
  { id: 1, vendedor: "Rota Cred Bank -", fecha: "2026-04-02", concepto: "Otros",       valor: 590.00,  obs: "IA que foi pago no cartão da mãe", evidencia: "" },
  { id: 2, vendedor: "Rota Cred Bank -", fecha: "2026-04-04", concepto: "Ajuste Caja", valor: 1300.00, obs: "",                                evidencia: "" },
  { id: 3, vendedor: "Rota Cred Bank -", fecha: "2026-04-10", concepto: "Otros",       valor: 200.00,  obs: "Pagamento de fornecedor",          evidencia: "" },
];

const retirosData = [
  { id: 1, vendedor: "Rota Cred Bank -", fecha: "2026-03-26", concepto: "Retiros de caja", valor: 250.00, obs: "" },
  { id: 2, vendedor: "Rota Cred Bank -", fecha: "2026-04-04", concepto: "Retiros de caja", valor: 1.00,   obs: "" },
  { id: 3, vendedor: "Rota Cred Bank -", fecha: "2026-05-25", concepto: "Retiros de caja", valor: 100.00, obs: "" },
];

function LiqPeriodosDespesasContent() {
  ...
  const thStyle: React.CSSProperties = {
    padding: "7px 10px", fontSize: 13, fontWeight: 700, color: "#e2e8f0",
    background: "#3d6e8e", borderRight: "1px solid #4a7fa0",
    textAlign: "left", whiteSpace: "nowrap", position: "sticky", top: 0, zIndex: 1,
  };
  const td = (extra?: React.CSSProperties): React.CSSProperties => ({
    padding: "6px 10px", borderRight: "1px solid #e5e7eb", borderBottom: "1px solid #f0f0f0",
    fontSize: 13, color: "#374151", whiteSpace: "nowrap", ...extra,
  });
  const sectionHeader = (label: string, open: boolean, toggle: () => void) => (
    <div
      ...
      style={{ background: "#eef2f7", borderBottom: "1px solid #d1d9e6", borderTop: "1px solid #d1d9e6" }}
    >
      <span style={{ fontSize: 13, color: "#2563eb", transform: open ? "rotate(0deg)" : "rotate(-90deg)", display: "inline-block", transition: "transform 0.15s" }}>▼</span>
      <span style={{ fontSize: 13, fontWeight: 700, color: "#1e3a5f" }}>{label}</span>
    </div>
  );

  const totalEgresos = egresosData.reduce((a, r) => a + r.valor, 0);
  const totalRetiros = retirosData.reduce((a, r) => a + r.valor, 0);
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" style={{ background: "#f8fafc" }}>
      <div className="flex-1 overflow-auto">
        ...
        {/* ── Egresos ── */}
        {sectionHeader("Egresos", openEgresos, () => setOpenEgresos(p => !p))}
        {openEgresos && (
          <table style={{ borderCollapse: "collapse", width: "100%", tableLayout: "fixed" }}>
            <colgroup>
              <col style={{ width: "14%" }} />
            ...
            <thead>
              <tr>
                {["Vendedor","Fecha","Concepto","Valor","Observaciones","Evidencias"].map(h => (
            ...
            <tbody>
              {egresosData.map((r, i) => (
                <tr key={r.id} style={{ background: i % 2 === 0 ? "#fff" : "#f9fafb", cursor: "pointer" }}
                  onMouseEnter={e => Array.from((e.currentTarget as HTMLTableRowElement).cells).forEach(c => c.style.background = "#eff6ff")}
                  onMouseLeave={e => { const bg = i%2===0?"#fff":"#f9fafb"; Array.from((e.currentTarget as HTMLTableRowElement).cells).forEach(c => c.style.background = bg); }}>
                  <td style={td({ color: "#374151", fontSize: 12 })}>{r.vendedor}</td>
                  <td style={td({ color: "#6b7280" })}>{r.fecha}</td>
                  <td style={td({ fontWeight: 600 })}>{r.concepto}</td>
                  <td style={td({ textAlign: "right", fontWeight: 700, color: "#b91c1c" })}>{fmt(r.valor)}</td>
                  <td style={td({ color: "#6b7280", fontStyle: r.obs ? "normal" : "italic" })}>{r.obs || "—"}</td>
                  <td style={td({ color: "#9ca3af", fontStyle: "italic" })}>{r.evidencia || "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {/* subtotal egresos */}
        {openEgresos && (
          <div className="flex justify-end px-4 py-1.5" style={{ background: "#f1f5f9", borderBottom: "1px solid #e2e8f0" }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#374151", marginRight: 16 }}>TOTAL EGRESOS</span>
            <span style={{ fontSize: 12, fontWeight: 800, color: "#b91c1c" }}>{fmt(totalEgresos)}</span>
          </div>
        )}

        {/* ── Retiros ── */}
        {sectionHeader("Retiros", openRetiros, () => setOpenRetiros(p => !p))}
        {openRetiros && (
          <table style={{ borderCollapse: "collapse", width: "100%", tableLayout: "fixed" }}>
            <colgroup>
              <col style={{ width: "16%" }} />
            ...
            <thead>
              <tr>
                {["Vendedor","Fecha","Concepto","Valor","Observaciones"].map(h => (
                  <th key={h} style={thStyle}>{h}</th>
                ))}
              </tr>
            </thead>
                ...
                <tr key={r.id} style={{ background: i % 2 === 0 ? "#fff" : "#f9fafb", cursor: "pointer" }}
                  ...
                  <td style={td({ fontWeight: 600, color: "#7c3aed" })}>
                    <span style={{ display:"inline-flex", alignItems:"center", gap:4 }}>
                      <svg viewBox="0 0 24 24" style={{ width:13, height:13, fill:"#7c3aed", flexShrink:0 }}><path d="M19 9h-4V3H9v6H5l7 7 7-7zM5 18v2h14v-2H5z"/></svg>
                      {r.concepto}
                    </span>
                  </td>
                  <td style={td({ textAlign: "right", fontWeight: 700, color: "#b91c1c" })}>{fmt(r.valor)}</td>
                  <td style={td({ color: "#9ca3af", fontStyle: "italic" })}>{r.obs || "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {openRetiros && (
          <div className="flex justify-end px-4 py-1.5" style={{ background: "#f1f5f9", borderBottom: "1px solid #e2e8f0" }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "#374151", marginRight: 16 }}>TOTAL RETIROS</span>
            <span style={{ fontSize: 12, fontWeight: 800, color: "#7c3aed" }}>{fmt(totalRetiros)}</span>
          </div>
        )}
      </div>
...
function LiqPeriodosContent({ activeSub }: { activeSub: string }) {
  ...
  if (activeSub === "Despesas")            return <LiqPeriodosDespesasContent />;
  return (
