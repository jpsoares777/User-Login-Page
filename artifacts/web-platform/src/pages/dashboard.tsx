...
import menuIcon from "@assets/menu-3d-icon-png-download-9571549_1776472972491.webp";
...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ fontFamily: "system-ui, sans-serif", background: "#f4f4f4" }}>
      ...
      {/* ── TOP BAR ── */}
      <div className="flex items-center justify-between h-12 px-3" style={{ background: "#2d5474" }}>
        <img
          src={menuIcon}
          alt="Menu"
        ...
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-1 px-3 h-7 text-sm font-semibold rounded text-white"
            style={{ background: "#c0392b" }}>▶ Tutoriais</button>
          <button className="px-3 h-7 text-sm font-semibold rounded text-white"
            style={{ background: "#e67e22" }}>Admin</button>
          <button className="flex items-center gap-1 px-3 h-7 text-sm font-medium rounded"
            style={{ background: "rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.8)" }}>
            🔑 Alterar Senha
          </button>
          <button onClick={() => navigate("/")}
            className="flex items-center gap-1 px-3 h-7 text-sm font-medium rounded"
