...
function Row({ label, children, bold, index = 0 }: { label: string; children: React.ReactNode; bold?: boolean; index?: number }) {
  ...
  return (
    <div
      className="flex flex-col sm:flex-row sm:items-center group transition-colors"
      style={{ fontSize: "12px", paddingLeft: "12px", background: even ? "#ffffff" : "#f9fafb", borderBottom: "1px solid #edf0f3" }}
      onMouseEnter={e => (e.currentTarget.style.background = "#eff6ff")}
      onMouseLeave={e => (e.currentTarget.style.background = even ? "#ffffff" : "#f9fafb")}
    >
      <span
        className={`shrink-0 pt-2 pb-0.5 sm:py-1 pr-3 whitespace-nowrap ${bold ? "font-semibold text-gray-700" : "text-gray-400 font-normal"}`}
        style={{ width: undefined, minWidth: "0", fontSize: bold ? "11px" : "11px" }}
      >
        {label}
      </span>
      <div className="hidden sm:block w-px self-stretch bg-gray-200 mr-3 shrink-0" />
      <span className={`pb-2 sm:py-1 px-1 flex items-center gap-1.5 leading-tight flex-wrap ${bold ? "font-bold text-gray-900 text-[13px]" : "text-gray-700"}`}>
        {children}
      </span>
    </div>
  );
...
const ACTION_BUTTONS = [
  ...
  { icon: "📊", label: "Relatório Monitor" },
  { icon: "👥", label: "Lista Clientes" },
  { icon: "🔒", label: "Bloquear Unidade" },
  { icon: "💰", label: "M. Juros" },
  { icon: "📈", label: "Ganância ( $0.00 )" },
];
...
export default function DashboardPage() {
  ...
  const ActionsPanel = () => (
    <div className="bg-gray-50 flex flex-col gap-1.5 p-2 overflow-y-auto h-full">
      {ACTION_BUTTONS.map(({ icon, label }) => (
        <button key={label}
          className="w-full text-left px-3 py-2.5 text-xs font-medium rounded text-white hover:opacity-90 active:opacity-75"
  ...
  return (
    <div className="h-[100dvh] flex flex-col overflow-hidden" style={{ fontFamily: "system-ui, sans-serif", background: "#f4f4f4" }}>
      ...
      <div className="flex items-center justify-between h-12 px-3 shrink-0" style={{ background: "#2d5474" }}>
        ...
        <div className="flex items-center gap-1.5">
          <button className="hidden sm:flex items-center gap-1 px-3 h-7 text-sm font-semibold rounded text-white" style={{ background: "#c0392b" }}>▶ Tutoriais</button>
          <button className="px-3 h-7 text-sm font-semibold rounded text-white" style={{ background: "#e67e22" }}>Admin</button>
          <button className="hidden sm:flex items-center gap-1 px-3 h-7 text-sm font-medium rounded" style={{ background: "rgba(255,255,255,0.12)", color: "rgba(255,255,255,0.8)" }}>🔑 Alterar Senha</button>
      ...
      {/* ── MAIN TABS ROW ── */}
      <div className="flex items-end px-2 gap-1 pt-1 shrink-0 overflow-x-auto scrollbar-none" style={{ background: "#2d5474" }}>
        {MAIN_TABS.map((tab) => (
          <button key={tab} onClick={() => setActiveMain(tab)}
            className="shrink-0 px-4 h-9 text-xs sm:text-sm font-medium transition-all rounded-t whitespace-nowrap"
            style={{
      ...
      {/* ── SUB TABS ROW ── */}
      <div className="flex items-center gap-1 px-2 py-1 shrink-0 overflow-x-auto scrollbar-none" style={{ background: "#e8edf2" }}>
        {SUB_TABS.map((tab) => (
          <button key={tab} onClick={() => setActiveSub(tab)}
            className="shrink-0 px-3 h-8 text-xs font-medium transition-all rounded whitespace-nowrap"
            style={{
      ...
      {/* ── FILTER BAR ── */}
      <div className="flex items-center h-10 px-3 gap-2 shrink-0" style={{ background: "#f8f9fa", borderBottom: "1px solid #e0e0e0" }}>
        <button className="flex items-center gap-1.5 px-2.5 h-7 text-xs font-medium rounded" style={{ background: "#2563eb", color: "#fff" }}>
          <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 fill-white opacity-90"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z"/></svg>
          País
        </button>
        <button className="flex items-center justify-center w-7 h-7 rounded" style={{ background: "#16a34a", color: "#fff" }}>
          <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 fill-white"><path d="M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"/></svg>
        </button>
        <button className="flex items-center gap-1.5 px-2.5 h-7 text-xs font-medium rounded" style={{ background: "#2563eb", color: "#fff" }}>
          <svg viewBox="0 0 24 24" className="w-3.5 h-3.5 fill-white opacity-90"><path d="M15.5 14h-.79l-.28-.27C15.41 12.59 16 11.11 16 9.5 16 5.91 13.09 3 9.5 3S3 5.91 3 9.5 5.91 16 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/></svg>
          Vendedor
          <span className="text-[10px] bg-white/25 rounded-full w-4 h-4 flex items-center justify-center font-bold">1</span>
        </button>
        <div className="flex-1" />
        <button className="flex items-center justify-center w-8 h-7 rounded" style={{ background: "#2563eb", color: "#fff" }}>
      ...
      <div className="flex-1 overflow-hidden flex relative">
        {showContent ? (
          <>
            {/* Mobile overlay backdrop for tree */}
            {showTree && (
              <div className="fixed inset-0 bg-black/40 z-20 md:hidden" onClick={() => setShowTree(false)} />
            )}
            {/* Mobile overlay backdrop for actions */}
            {showActions && (
              <div className="fixed inset-0 bg-black/40 z-20 md:hidden" onClick={() => setShowActions(false)} />
            )}

            {/* LEFT: Tree Panel — desktop always visible, mobile slide-in */}
            <div
              className={`
                fixed md:relative inset-y-0 left-0 z-30 md:z-auto
                w-72 md:w-64 shrink-0 border-r border-gray-200
                transition-transform duration-300 ease-in-out
                ${showTree ? "translate-x-0" : "-translate-x-full md:translate-x-0"}
              `}
              style={{ top: 0, bottom: 0 }}
            >
              {/* Mobile header for tree drawer */}
              <div className="flex items-center justify-between px-3 py-2 md:hidden" style={{ background: "#2d5474" }}>
                <span className="text-white text-sm font-semibold">Estrutura</span>
                <button onClick={() => setShowTree(false)} className="text-white/70 hover:text-white text-lg leading-none">✕</button>
              </div>
              <TreePanel />
            </div>

            {/* CENTER: Data rows — always full width on mobile */}
            <div className="flex-1 overflow-y-auto border-r border-gray-200 pb-16 md:pb-0" style={{ background: "#f8fafc" }}>
            ...
            {/* RIGHT: Actions — desktop always visible, mobile slide-in from right */}
            <div
              className={`
                fixed md:relative inset-y-0 right-0 z-30 md:z-auto
                w-56 md:w-48 shrink-0
                transition-transform duration-300 ease-in-out
                ${showActions ? "translate-x-0" : "translate-x-full md:translate-x-0"}
              `}
              style={{ top: 0, bottom: 0 }}
            >
              <div className="flex items-center justify-between px-3 py-2 md:hidden" style={{ background: "#2d5474" }}>
                <span className="text-white text-sm font-semibold">Ações</span>
                <button onClick={() => setShowActions(false)} className="text-white/70 hover:text-white text-lg leading-none">✕</button>
              </div>
              <ActionsPanel />
            </div>

            {/* ── MOBILE BOTTOM NAV BAR ── */}
            <div className="md:hidden fixed bottom-0 left-0 right-0 z-20 flex items-center justify-around h-14 border-t"
              style={{ background: "#2d5474", borderColor: "rgba(255,255,255,0.1)" }}>
              <button onClick={() => { setShowTree(t => !t); setShowActions(false); }}
                className="flex flex-col items-center gap-0.5 px-4 py-1 rounded text-white/70 hover:text-white active:text-white transition-colors">
                <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current"><path d="M3 18h18v-2H3v2zm0-5h18v-2H3v2zm0-7v2h18V6H3z"/></svg>
                <span className="text-[10px] font-medium">Estrutura</span>
              </button>
              <button
                className="flex flex-col items-center gap-0.5 px-4 py-1 rounded text-white/70 active:text-white transition-colors"
                style={{ color: "rgba(255,255,255,0.4)" }}
              >
                <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 17.93V18c0-.55-.45-1-1-1s-1 .45-1 1v1.93C7.06 19.44 4.56 16.94 4.07 14H6c.55 0 1-.45 1-1s-.45-1-1-1H4.07C4.56 8.06 7.06 5.56 10 5.07V7c0 .55.45 1 1 1s1-.45 1-1V5.07c2.94.49 5.44 2.99 5.93 5.93H16c-.55 0-1 .45-1 1s.45 1 1 1h1.93c-.49 2.94-2.99 5.44-5.93 5.93z"/></svg>
                <span className="text-[10px] font-medium">Início</span>
              </button>
              <button onClick={() => { setShowActions(a => !a); setShowTree(false); }}
                className="flex flex-col items-center gap-0.5 px-4 py-1 rounded text-white/70 hover:text-white active:text-white transition-colors">
                <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 17.93V18c0-.55-.45-1-1-1s-1 .45-1 1v1.93C7.06 19.44 4.56 16.94 4.07 14H6c.55 0 1-.45 1-1s-.45-1-1-1H4.07C4.56 8.06 7.06 5.56 10 5.07V7c0 .55.45 1 1 1s1-.45 1-1V5.07c2.94.49 5.44 2.99 5.93 5.93H16c-.55 0-1 .45-1 1s.45 1 1 1h1.93c-.49 2.94-2.99 5.44-5.93 5.93z"/></svg>
                <span className="text-[10px] font-medium">Ações</span>
              </button>
            </div>
          </>
        ) : (
